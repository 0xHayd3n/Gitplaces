...
describe('useLastCommits', () => {
  it('returns undefined for paths not yet fetched', () => {
    const { result } = renderHook(() => useLastCommits({ hostId: HOST_ID_GITHUB, repoId: '1', owner: 'o', name: 'n', ref: 'main' }))
  ...
  it('fetches and stores last-commit info for a requested path', async () => {
    ...
    ;(window.api.repo.getLastCommitsForPaths as ReturnType<typeof vi.fn>).mockResolvedValue({ 'src/foo.ts': info })
  ...
  it('does not refetch a path that is already cached', async () => {
    ...
    const mockFn = window.api.repo.getLastCommitsForPaths as ReturnType<typeof vi.fn>
  ...
  it('handles null results without erroring', async () => {
    ;(window.api.repo.getLastCommitsForPaths as ReturnType<typeof vi.fn>).mockResolvedValue({ 'src/foo.ts': null })
    const { result } = renderHook(() => useLastCommits({ hostId: HOST_ID_GITHUB, repoId: '1', owner: 'o', name: 'n', ref: 'main' }))
    result.current.request([{ path: 'src/foo.ts', sha: 'sha1' }])
    await waitFor(() => {
