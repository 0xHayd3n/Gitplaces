...
describe('Phase 26 migration — body → primary file', () => {
  ...
  it('backfills a primary file row when an agent is added pre-migration', () => {
    // freshDb already ran Phase 26 on an empty agents table.
    // Insert an agent directly (no primary file row yet), then re-run
    // initSchema to trigger Phase 26's backfill loop on the new agent.
  ...
  it('shifts any pre-existing sibling at sort_order = 0 to sort_order = 1', () => {
    ...
    // Manually insert a sibling sitting at sort_order=0 (simulating pre-existing
    // data; the UI never produces this state but the migration must handle it).
    db.prepare(`
  ...
  it('promotes an existing <handle>.md to sort_order=0 when it sits at a non-zero position', () => {
    ...
    // Pre-existing file with the primary filename but at sort_order=2.
    db.prepare(`
      INSERT INTO agent_files (id, agent_id, filename, content, sort_order, created_at, updated_at)
    ...
    expect(row.id).toBe('preex')                      // existing row promoted, not replaced
    expect(row.filename).toBe('promo.md')
    expect(row.content).toBe('pre-existing content')  // user content preserved
  })
